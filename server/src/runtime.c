  ...
  for (i = 0; i < lp->rx.n_nic_queues; i++) {
    ...
    for (j = 0; j + 3 < n_mbufs; j += 2) {
      ...
      app_lcore_io_rx_buffer_to_send(
          lp, worker_1, mbuf_0_1,
          bsz_wr); // delivers packets to different cores
    }
...
int app_lcore_main_loop(__attribute__((unused)) void *arg) {
  ...
  if (lp->type == e_APP_LCORE_WORKER) { // app_get_lcores_worker uses the same
                                        // check, so it's alright to open db in
                                        // the main loop for the workers
