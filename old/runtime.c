...
static inline void app_lcore_io_rx(struct app_lcore_params_io *lp,
  ...
  for (i = 0; i < lp->rx.n_nic_queues; i++) {
    ...
    n_mbufs =
        rte_eth_rx_burst(port, queue, lp->rx.mbuf_in.array, (uint16_t)bsz_rd);
    ...
    //
    //             printf("I/O RX %u in (NIC port %u): NIC drop ratio = %.2f avg
    //             burst size = %.2f\n",
    //                    lcore,
    //                    port,
    //                    (double)stats.imissed / (double)(stats.imissed +
    //                    stats.ipackets),
    ...
    for (j = 0; j + 3 < n_mbufs; j += 2) {
      ...
      app_lcore_io_rx_buffer_to_send(lp, worker_1, mbuf_0_1, bsz_wr); // delivers packets to different cores
    }
  ...
  for (worker = 0; worker < n_workers; worker++) {
    ...
    for (i = 0; i < lp->tx.n_nic_ports; i++) {
      ...
      ret = rte_ring_sc_dequeue_bulk(
          ring, (void **)&lp->tx.mbuf_out[port].array[n_mbufs], bsz_rd, NULL);
      ...
      n_pkts = rte_eth_tx_burst(port, 0, lp->tx.mbuf_out[port].array,
                                (uint16_t)n_mbufs);
      ...
      //                 unsigned lcore = rte_lcore_id();
      //
      //                 printf("\t\t\tI/O TX %u out (port %u): avg burst size =
      //                 %.2f\n",
      //                        lcore,
      //                        port,
      //                        ((double)lp->tx.nic_ports_count[port]) /
      //                        ((double)lp->tx.nic_ports_iters[port]));
...
static inline void app_lcore_worker(struct app_lcore_params_worker *lp,
  ...
  for (i = 0; i < lp->n_rings_in; i++) {
    ...
    for (j = 0; j < bsz_rd; j++) {
      ...
      ret = rte_ring_sp_enqueue_bulk(
          lp->rings_out[port], (void **)lp->mbuf_out[port].array, bsz_wr, NULL);
      ...
      //             {
      //                 printf("\t\tWorker %u out (NIC port %u): enq success
      //                 rate = %.2f\n",
...
static void app_lcore_main_loop_worker(void) {
  ...
  // printf("%s\n", "1");
  // //this is where we should open the databases
  // char db_path[MAX_NB_PARTITION];
  // // char * db_path;
  // sprintf(db_path,"%d", lcore);
  // int ret = open_database(lp, db_path, app.options, app.txdb_options);
  // printf("%s\n", "1");
  // printf("%i\n", ret);
  // if (ret < 0) {
  //   printf("Unable to open db\n");
  // }
  // open database assigns db_path, & pointer to db
...
int app_lcore_main_loop(__attribute__((unused)) void *arg) {
  ...
  if (lp->type == e_APP_LCORE_WORKER) { // app_get_lcores_worker uses the same check, so it's alright to open db in the main loop for the workers
